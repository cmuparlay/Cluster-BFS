...
struct graph_utils {
  ...
  static graph read_graph_from_bytepd(const std::string& filename){
    ...
    ifs.read(reinterpret_cast<char*>(&m), sizeof(long));
    // ifs.read(reinterpret_cast<char*>(&sizes), sizeof(long));
    // assert(sizes == (n + 1) * 8 + m * 4 + 3 * 8);
    printf("n:%ld m:%ld\n", n, m);
    parlay::sequence<uint64_t> degree(n);
    parlay::sequence<uint64_t> edge(m);
    ifs.read(reinterpret_cast<char*>(degree.begin()), (n) * 8);
    ifs.read(reinterpret_cast<char*>(edge.begin()), (m) * 8);
    if (ifs.peek() != EOF) {
      std::cerr << "Error: Bad data\n";
      abort();
    }
    ifs.close();
    auto [offsets,total] = scan(degree);
    printf("offsets total: %ld\n", total);
    offsets.push_back(m);
    auto edges = parlay::map(edge, [&](uint64_t e){return (vertex)e;});
    return parlay::tabulate(n, [&, o=offsets.begin()] (vertex i) {
  ...
  static graph read_graph_from_bin(const std::string& filename){
    ...
    auto edges = parlay::tabulate(m, [&](uint64_t i){return (vertex)edge[i];});
    if (ifs.peek() != EOF) {
      std::cerr << "Error: Bad data\n";
      abort();
    }
  ...
  static void write_binary_format(const graph& G, const std::string& filename) {
    using EdgeId = uint64_t;
    using NodeId = uint32_t;
    size_t m = parlay::reduce(parlay::map(G,parlay::size_of()));
    size_t n = G.size();
    size_t sizes = (n + 1) * 8 + m * 4 + 3 * 8;
    auto offsets = parlay::map(G, [] (auto& nghs) {return (EdgeId) nghs.size();});
    parlay::scan_inplace(offsets);
    offsets.push_back((EdgeId)m);
    auto tmp_edges = flatten(G);
    printf("offsets size: %lu\n", offsets.size());
    printf("tmp_edges size: %lu\n", tmp_edges.size());
    printf("print 2147483648 latter edges: %d\n");
    for (size_t i = 2147483648; i<2147483648+100; i++){
      printf("%u ",tmp_edges[i]);
    }
    printf("\n");
  ...
  static graph random_graph(vertex n, int k){
    parlay::sequence<std::pair<vertex, vertex>> pairs(k*2*n);
    parlay::parallel_for(0, n, [&](vertex i){
      for (int j =2*k*i;j<2*k*(i+1);j+=2){
        vertex v = parlay::hash32(j)%n;
        pairs[j]=std::pair(i,v);
        pairs[j+1]=std::pair(v,i);
      }
    });
    parlay::sort_inplace(pairs);
    auto filter = parlay::tabulate(k*2*n, [&](vertex i){return (pairs[i].first!=pairs[i].second) && (i==0 || (pairs[i].first!=pairs[i-1].first)||pairs[i].second != pairs[i-1].second);});
    auto edges=parlay::pack(pairs, filter);
    parlay::sequence<parlay::sequence<vertex>> r(n);
    size_t m = edges.size();
    auto iota = parlay::internal::delayed_tabulate(m, [&](vertex i) { return i; });
    auto starts = parlay::block_delayed::filter(iota, [&] (size_t i) {
    return (i==0) || parlay::internal::get_key(edges[i-1]) < parlay::internal::get_key(edges[i]);});
    parlay::parallel_for(0, n, [&] (size_t i) {
      size_t start = starts[i];
      size_t end = ((i == n-1) ? m : starts[i+1]);
      r[std::get<0>(edges[starts[i]])] =
      parlay::map(edges.cut(start, end), [&] (auto kv) {
      return std::move(parlay::internal::get_val(kv));}, 1000);});
    return r;
  }
};
